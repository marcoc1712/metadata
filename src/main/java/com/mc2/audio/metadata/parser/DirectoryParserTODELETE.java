 ...
 * @author marco
 */
public class DirectoryParserTODELETE {
	private static Logger log = Logger.getLogger(DirectoryParserTODELETE.class.getName()); 
    ...
    public static AlbumDefaultImpl parse(File directory) throws IOException,  InvalidAudioFileException, InvalidAudioFileFormatException{
					...
					for (FileData fileData : cueFile.getCuesheet().getFileDataList()){
						...
						for (TrackData trackData : fileData.getTrackDataList() ){
							...
							if ( track != null){
								...
								track = new TrackDefaultImpl(trackData.getTrackId(), merge(Section.TRACK,fromAlbum, trackData.getMetadata()), trackCoverArt);
				...
				if (audiofile != null){
					...
					if (!trackId.isEmpty()) {
						...
						if ( track == null){
							...
							track = new TrackDefaultImpl(trackId, merge(Section.TRACK,new ArrayList<>(),audiofile.getMetadata()), trackCoverArt);
       ...
       */
        ...
        AlbumDefaultImpl out = new AlbumDefaultImpl(url, coverArtList, atAlbumLevel, tracklist, directoryfileList, cueFileList, audioFileList, imagefileList, statusMessageList, new ArrayList<>());
